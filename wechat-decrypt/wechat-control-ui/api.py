...
def list_targets(kind: str = "all", base_url: str = DEFAULT_BASE_URL) -> List[Dict[str, Any]]:
    res = _request("GET", "/targets", base_url=base_url, params={"kind": kind})
...
def create_agent_test_job(prompt: str, provider: str = "echo",
                          ...
                          sender: str = "tester",
                          base_url: str = DEFAULT_BASE_URL) -> Dict[str, Any]:
    return _request("POST", "/agent/jobs/test", base_url=base_url, body={
        "prompt": prompt,
        "provider": provider,
        "group_key": group_key,
        "sender": sender,
...
def agent_provider_health(provider: str = "genericagent",
    ...
    res = _request("GET", "/agent/provider/health", base_url=base_url,
                   params={"provider": provider}, timeout=10)
...
def start_agent_instance(instance_id: str, timeout: float = 240, worker_count: int = 1,
                         base_url: str = DEFAULT_BASE_URL) -> Dict[str, Any]:
    return _request("POST", "/agent/instance/%s/start" % instance_id,
