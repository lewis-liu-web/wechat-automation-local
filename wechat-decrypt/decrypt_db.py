...
def _safe_target_rel(db_rel):
    rel = db_rel.replace("\\", "/").lstrip("/")
    norm = os.path.normpath(rel)
    if norm.startswith("..") or os.path.isabs(norm):
        raise ValueError(f"不安全的数据库相对路径: {db_rel}")
    return norm


def _collect_db_files(db_dir, target_rel=None):
    db_files = []
    if target_rel:
        rel = _safe_target_rel(target_rel)
        path = os.path.join(db_dir, rel)
        if not os.path.exists(path):
            raise FileNotFoundError(f"目标数据库不存在: {path}")
        if not rel.endswith(".db") or rel.endswith("-wal") or rel.endswith("-shm"):
            raise ValueError(f"目标不是普通 .db 文件: {rel}")
...
def main(argv=None):
    ...
    parser.add_argument("--db", dest="target_db", help="只解密指定相对路径数据库，例如 message/message_0.db")
    parser.add_argument("--out-dir", default=OUT_DIR, help=f"输出目录，默认: {OUT_DIR}")
    parser.add_argument("--keys-file", default=KEYS_FILE, help=f"密钥索引文件，默认: {KEYS_FILE}")
    parser.add_argument("--db-dir", default=DB_DIR, help=f"原始数据库目录，默认来自 config.json")
    ...
    if args.target_db:
        print(f"模式: 单库解密 ({args.target_db})")
    else:
        print("模式: 全库解密")

    # 加载密钥；只加载映射，不打印密钥内容
    if not os.path.exists(args.keys_file):
        print(f"[ERROR] 密钥文件不存在: {args.keys_file}")
    ...
    try:
        db_files = _collect_db_files(args.db_dir, args.target_db)
    except Exception as e:
        print(f"[ERROR] 收集数据库失败: {e}")
        sys.exit(2)
