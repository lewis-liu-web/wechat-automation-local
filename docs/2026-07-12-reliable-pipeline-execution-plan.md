...
1. **双路径风险**：当前 monitor 同时存在 legacy 路径与 reliable path。Stage 3 必须显式移除 legacy 调用点（`wechat_bot_monitor.py:1675-1707`、`1801-1803`）中已启用目标对旧 `generate_reply` 的调用，但保留 per-target gate 作为未启用目标的迁移开关；Stage 4 在全部切换后再删除该 gate。
...
| Stage 2 | Hermes 统一契约产出 | 未开始 | 同步/异步等价、严格契约测试 |
