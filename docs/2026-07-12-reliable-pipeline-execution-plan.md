...
| Stage 2 Phase A<br>Consumer-side contract convergence (foreground agent_worker, strict async reconciler jobs, reliable_worker) | **已提交并部署** | commit `88ee131 Unify strict AgentResult consumers` 已随 Stage 2 runtime sync 部署；其 consumer-side strict contract 由 Phase B E2E 一并验证 |
| Stage 2 Phase B<br>Hermes provider default/always strict + legacy extractor retirement | **已完成** commit `5b3e196 Make Hermes provider strict-only`；已部署/E2E | Hermes `run`/`submit`/`poll` strict-only；legacy stdout/parser 与 Python tool loop 已退役；173 条聚焦测试、生产模块编译、configured profile strict preflight、`bot群聊测试` E2E 均通过；全仓 collect-only 仍被既有 `wechat_auto` 缺失阻断 |
...
- **下一阶段**：
  - Stage 3：提取 `knowledge_retrieval.py`，迁移 KB/回复决策到 Hermes；
  - 全仓 collect-only 仍有既有 `wechat_auto` import 缺失，作为独立仓库健康项处理。
...
| Stage 2 整体 | 统一 Hermes runner/strict contract 产出 | **已完成** | Phase A + Phase B 已提交/部署，strict profile preflight 和 E2E 通过 |
